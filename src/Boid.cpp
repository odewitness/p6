...
std::vector<Boid> creation_boids(int num_boids, float limite_haut, float limite_bas, float limite_gauche, float limite_droite, float taille_boid, float limite_devant, float limite_derriere)
{
    ...
    {
        glm::vec3 position  = glm::vec3(p6::random::number(limite_gauche, limite_droite), p6::random::number(limite_bas, limite_haut), p6::random::number(limite_derriere, limite_devant));
        ...
        BoidProperties properties = {position, direction, vitesse, taille_boid, limite_haut, limite_bas, limite_gauche, limite_droite, limite_devant, limite_derriere, color};
...
void Boid::cohesion(std::vector<Boid>& boids, const float& cohesion_rayon, const float& cohesion_force)
{
    ...
    if (compteur_voisin_proximite > 0)
    {
        ...
        glm::vec3 vecteur_cohesion = centre_boids - m_properties.position;
...
{
    glm::vec3 direction_moyenne(0.0f);
    int       compteur_voisin_proximite = 0;
...
{
    glm::vec3 separation_moyenne(0.0f);
    int       compteur_voisin_proximite = 0;
